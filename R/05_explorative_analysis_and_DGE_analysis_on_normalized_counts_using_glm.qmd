...
pca_plot_hsc <- ggplot(data = augmented, 
  ...
  geom_point(alpha = 0.7, 
             ...
             shape = 21, 
             color = "black") + 
  labs(title = "PCA\n HSC samples without VST transformation", 
       x = "PC1", 
       y = "PC2", 
       color = "Diagnosis") + 
  theme_prism() +
...
pca_data_for_plot <- pca_data[[1]]
```

We use these data for plotting

```{r}
ggplot(data = pca_data_for_plot, 
       mapping = aes(x = PC1, 
                     y = PC2, 
                     fill = diagnosis)
       ) + 
  ...
  theme_prism() + 
  scale_fill_brewer(palette = "Set1") + 
  labs(title = "PCA\n HSC samples with VST transformation")
```

# Differential gene expression analysis using data from the DESeq2 analysis.

We get the estimations of differential gene expression using the `results` function.
...
Looking at expression of immunosuppressive proteins.
...
ggplot(data = df_tma, 
  ...
  geom_hline(yintercept = -log10(0.05),
             color = "red") + 
  theme_prism() +
  labs(title = "Expression of immunosuppressive proteins\n in CD34+ HSC from MDS patients and healthy donors", 
...
ggplot(data = sign_genes, 
  ...
  labs(y = "Gene name", 
       ...
       subtitle = "Analysis performed using linear regression on normalized counts") +
