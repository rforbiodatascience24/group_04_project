...
count_data <- read_rds(file = "data/02_count_data_augments.Rdata")
```

To establish the DESeq2 object we use the `DESeqDataSetFromMatrix` function and create the object `dds_object`. 
We stratify according to `diagnosis`, `sf3b1`, `srsf2` and `cells` where `diagnosis` is put in the end of the argument `design`
```{r}
...
The genes are in the rownames, and we want these to be a separate column.
First we convert from matrix to dataframe. 
```{r}
...
Data saved 
```{r}
write_tsv(x = full_data, file = "data/03_full_data.tsv")
```
