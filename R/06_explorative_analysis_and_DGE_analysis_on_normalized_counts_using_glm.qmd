...
#Creating DESeq Dataset - using only diagnosis in the design because sf3b1 and srsf2 are perfectly correlated for this cell type tissues.
...
ggplot(data = gr_df, 
  ...
  geom_hline(yintercept = -log10(0.000001), 
             color = "red") + 
  geom_point(alpha = 0.1) + 
  geom_label_repel(data = gr_df |> 
                     filter(padj < 0.000001 & abs(log2FoldChange) > 10
                            ), 
                   aes(label = gene), 
                   max.overlaps = 20000, 
                   nudge_x = 2) + 
  theme_prism() + 
  labs(title = "Differential gene expression in CD34+ Granulocytic\n between patients with MDS and healthy donors", 
       caption = "Red line indicates an adjusted p-value of 0.000001\n
       labelled genes with a significance value below 0.000001")
```
...
ggplot(data = mo_df, 
  ...
  geom_hline(yintercept = -log10(p_value), 
             color = "red") + 
  geom_point(alpha = 0.1) + 
  geom_label_repel(data = mo_df |> 
                     filter(padj < p_value #& abs(log2FoldChange) > 7.5
                            ), 
                   aes(label = gene), 
                   max.overlaps = 20000, 
                   nudge_x = 2) + 
  theme_prism() + 
  labs(title = "Differential gene expression in CD34+ Monocytic\n between patients with MDS and healthy donors", 
       caption = "Red line indicates an adjusted p-value of 0.001\n
       labelled genes with a significance value below 0.001")
```
...
ggplot(data = er_df, 
  ...
  geom_hline(yintercept = -log10(p_value), 
             color = "red") + 
  geom_point(alpha = 0.1) + 
  geom_label_repel(data = er_df |> 
                     filter(padj < p_value & abs(log2FoldChange) > 10
                            ), 
                   aes(label = gene), 
                   max.overlaps = 20000, 
                   nudge_x = 2) + 
