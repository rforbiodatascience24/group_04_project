...
pca_plot_all <- ggplot(data = augmented, 
  ...
  labs(title = "PCA of all samples - non-VST normalized", 
       x = "PC1", 
       y = "PC2", 
       shape = "Cell fraction", 
       color = "Diagnosis") + 
...
## PCA of HSC only using VST transformed data
...
df_pca_all_cells <- pca_all_cells[[1]]
...
```{r}
pca_all_cells_vst <- ggplot(data = df_pca_all_cells_joined, 
                            mapping = aes(x = PC1, 
                                          y = PC2, 
                                          shape = cells, 
                                          color = diagnosis.x)) + 
  ...
  scale_color_brewer(palette = "Set1") + 
  labs(title = "PCA of all samples - VST transformed",
       color = "Diagnosis", 
       shape = "Cells") +
