...
```{r}
count_data <- read_tsv(file = "data/01_cleaned_count_data.tsv")
```

Firstly, we need to map the identifier in the `GeneID` variable with a proper gene name. The GeneID is an EntrezID, so we need to convert the EntrezID into a proper gene name, so that the analyses make biological sense to us.
...
count_data <- count_data |> 
  ...
  relocate(.after = GeneID)
```

We make a dataframe termed `conversion_check` which includes `GeneID` and `gene` and we use this to manually check the EntrezID of some genes

```{r}
conversion_check <- count_data |> 
  dplyr::select(GeneID, gene)

View(conversion_check)
```
...
So of the `nrow(count_data)` rows in our `count_data`, `sum(is.na(count_data$gene))` are not converted to a gene symbol.
As these could corrupt our later analysis we chose to remove observations - e.g. gene names that have `NA` in their `gene`
...
We can use the `rownames_to_column` function, but check for duplicates in the `gene` column as we cannot make the function work if there are duplicates.
So we check the `sum` of all duplicated genes and find that there is `sum(duplicated(count_data$gene))` duplicate gene.
...
However we cannot bind the vector `trnav_cac_mean` to the `count_data` directly, as the vector does not contain the `gene` variable. 
This needs to be solved. If not we should continue with our analysis without this one gene "TRNAV-CAC". 
...
Now the data is in the correct format for the `DESeq2` analysis and we save it. 
Notably, the data is saved as an R-object, as the rownames would otherwise be lost in conversion, which is not desirable when working with `DESeq2`. 

```{r}
write_rds(x = count_data, file = "data/02_count_data_augments.Rdata")
```
