...
However we cannot bind the vector `trnav_cac_mean` to the `count_data` directly, as the vector does not contain the `gene` variable. This needs to be solved. If not we should continue with our analysis without this one gene "TRNAV-CAC".
