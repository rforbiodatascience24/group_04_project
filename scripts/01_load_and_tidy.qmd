...
```{r}
meta_data <- read_tsv(file = "data/raw_data/GSE114922_family.soft.gz")
```
...
We remove `accession` as this is the same as `sample`and the `source` variable is renamed into `cells`variable with shorter names. 
`Myelodysplastic syndrome` recoded to `MDS` and `Healthy donor` to `HD`. 
```{r}
sample_data <- sample_data |> 
  select(-accession) |> 
